    ...
    for api in apis:
        try:
            async with aiohttp.ClientSession() as session:
                ...
                if api["method"] == "POST":
                    async with session.post(api["url"], data=api["data"](link), headers=headers, timeout=15) as response:
                        if response.status == 200:
                            try:
...
async def instagram_handler(Mbot, message):
    ...
    try:
        ...
        if not result:
            dd_url = link.replace("instagram.com", "ddinstagram.com")
            fail_text = "Download Failed

Try this direct link:
" + dd_url + "

Original: " + link
            await status_msg.edit(fail_text)
            return
...
{footer}" if caption else footer
        if result["type"] == "file":
            ...
            sent_msg = await message.reply_video(result["path"], caption=final_caption)
            if INSTA_CHANNEL:
                try:
...
{user_info}")
            ...
            big_file_text = f"File is larger than 50MB

{final_caption}"
            await status_msg.edit(big_file_text, reply_markup=btn, disable_web_page_preview=True)
    except Exception as e:
